<section>
            ...
            <div class="card-body bg-gray-100">
                @if ($requirement->id == $item->id)
                    <form wire:submit.prevent='update'>
                        <label for="requirement.name" class="text-sm">Editando Requerimiento - <span class="italic"> al
                                finalizar
                                presiona: <span class="font-bold">Enter</span></span></label>
                        <input wire:model="requirement.name" type="text"
                            class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300  rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"
                            id="requirement.name" placeholder="Ingrese un Requerimiento">
                    </form>
                    <strong class="text-red-600 text-xs">{{ $errors->first('requirement.name') }}</strong>
                @else
    ...
    <div x-data="{ open: false }">
        ...
        <article class="card" x-show="open">
            <div class="card-body bg-gray-100">
                <h1 class="text-xl font-bold mb-4">Añadiendo nuevo requerimiento</h1>
                <div class="mb-4">
